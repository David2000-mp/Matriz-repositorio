# CHAMPILEAKS - Maristas Analytics Platform
...
CHAMPILEAKS es una plataforma de inteligencia digital diseñada para la Red Marista, que permite el análisis y seguimiento de métricas de redes sociales de instituciones educativas maristas en México.
...
Para cargar la base de datos completa desde Google Sheets:
...
2. **Configurar credenciales de Google Cloud Platform**:
   - Crea una Service Account en GCP
   - Descarga el JSON de credenciales
   - Completa las variables en `.env`:
     - `GOOGLE_SHEETS_ID`: ID del spreadsheet
     - `GCP_PRIVATE_KEY`, `GCP_CLIENT_EMAIL`, etc.

3. **Configuración persistente (opcional)**:
   ```powershell
   # Ejecutar como administrador
   .\configure_env.ps1 -SheetsId 'TU_SHEETS_ID' -PrivateKey 'TU_PRIVATE_KEY' ...
   ```

4. **Generar datos de ejemplo adicionales**:
   ```bash
...
Si no configuras Google Sheets, la aplicación usará datos de ejemplo locales (39 cuentas, 234 métricas).
...
**CHAMPILEAKS** - Inteligencia Digital Marista 🎓
