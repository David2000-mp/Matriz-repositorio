...
def test_load_data_conexion_exitosa_devuelve_dos_dataframes(
    mock_conectar_sheets, sample_cuentas_df, sample_metricas_df
):
    """
...
def test_load_data_normaliza_id_cuenta_a_lowercase(mock_conectar_sheets):
    ...
    assert all(
        df_cuentas["id_cuenta"] == df_cuentas["id_cuenta"].str.lower()
    ), "Todos los id_cuenta deben estar normalizados a lowercase"

    assert all(
        df_metricas["id_cuenta"] == df_metricas["id_cuenta"].str.lower()
...
def test_load_data_dataframes_tienen_tipos_correctos(mock_conectar_sheets):
    ...
    if not df_metricas.empty:
        ...
        assert pd.api.types.is_numeric_dtype(
            df_metricas["seguidores"]
        ), "Columna 'seguidores' debe ser numérica"
        assert pd.api.types.is_numeric_dtype(
            df_metricas["interacciones"]
...
def test_get_id_devuelve_id_existente_si_cuenta_ya_existe(
    mock_conectar_sheets, sample_cuentas_df
):
    """
...
@pytest.mark.unit
def test_get_id_es_case_insensitive(mock_conectar_sheets, sample_cuentas_df):
    """
...
@pytest.mark.unit
def test_get_id_normaliza_id_a_lowercase(mock_conectar_sheets, sample_cuentas_df):
    """
...
def test_guardar_datos_llama_a_sheets_correctamente(
    mock_conectar_sheets, sample_cuentas_df, sample_metricas_df, monkeypatch
):
    """
...
def test_conectar_sheets_devuelve_spreadsheet(mock_streamlit_secrets):
    ...
    with (
        patch("utils.data_manager.Credentials") as mock_creds_class,
...
@pytest.mark.skip(
    reason="Test aislado - ejecutar solo: pytest -k 'test_conectar_sheets_maneja_falta_de_credenciales'"
)
@pytest.mark.unit
def test_conectar_sheets_maneja_falta_de_credenciales():
    ...
    with (
        patch("streamlit.secrets", mock_secrets),
        patch("streamlit.error"),
    ):  # Silenciar st.error() durante test
        # ACT
        resultado = conectar_sheets()
    ...
    mock_data = [
        {
            "entidad": "Instituto C",
            "plataforma": "Twitter",
            "usuario_red": "@institutoC",
        },
        {
            "entidad": "Instituto D",
            "plataforma": "LinkedIn",
            "usuario_red": "@institutoD",
        },
    ]
