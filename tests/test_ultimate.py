...
def test_load_data_fallback_csv_exitoso(monkeypatch, caplog, clean_data_manager):
    ...
    df_cuentas = pd.DataFrame({
        'id_cuenta': ['test1'], 'entidad': ['Colegio Test'], 
        'plataforma': ['FB'], 'usuario_red': ['@user']
    })
    df_metricas = pd.DataFrame({
        'id_cuenta': ['test1'], 'fecha': [pd.Timestamp('2024-01-01')], 
        'seguidores': [100], 'alcance': [500], 'interacciones': [10],
        'likes_promedio': [1], 'engagement_rate': [0.1]
    })
    monkeypatch.setattr(dm, "init_files", lambda: None)
    monkeypatch.setattr(dm, "conectar_sheets", MagicMock(side_effect=Exception("Fallo Critico API")))
...
def test_conectar_sheets_maneja_error_gspread(monkeypatch, clean_data_manager):
    ...
    monkeypatch.setattr("streamlit.secrets", {"gcp_service_account": {"type": "service"}})
