...
def render():
    ...
    if resumen.empty:
        ...
        # Gráficas primero
        tab_vol, tab_qual = st.tabs(
            ["Volumen (Seguidores/Interacciones)", "Calidad (Engagement)"]
        )
        with tab_vol:
            fig_vol = px.line(
                resumen,
    ...
    estado_global = st.session_state.get(
        "institucion_activa", "Todas las Instituciones"
    )
    ...
    entidad = st.selectbox(
        "Seleccionar Institución", lista_colegios, index=default_index
    )
    ...
    er_network = (
        (
            df_network_last["interacciones"].sum()
            / df_network_last["seguidores"].sum()
            * 100
        )
        if df_network_last["seguidores"].sum() > 0
        else 0
    )

    # Deltas vs red
    delta_seg = (
        ((seg_inst - avg_seg_network) / avg_seg_network * 100)
        if avg_seg_network > 0
        else 0
    )
    delta_int = (
        ((int_inst - avg_int_network) / avg_int_network * 100)
        if avg_int_network > 0
        else 0
    )
    ...
    with tab_a:
        ...
        df_network_avg = (
            ...
            .agg(
                {
                    "seguidores": lambda x: x.groupby(df.loc[x.index, "entidad"])
                    .max()
                    .mean()
                }
            )
    ...
    with tab_b:
        ...
        df_network_er = (
            df.groupby(["fecha", "plataforma"])["engagement_rate"].mean().reset_index()
        )
